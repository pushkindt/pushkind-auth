...
use pushkind_auth::domain::types::{
    HubName, MenuName, MenuUrl, RoleName, UserEmail, UserName, UserPassword,
};
...
fn test_user_repository_crud() {
    ...
    let new_user = NewUser::new(
        ...
        Some(UserName::new("TestUser").unwrap()),
        hub.id,
        UserPassword::new("test").unwrap(),
    );
    let user = user_repo.create_user(&new_user).unwrap();
    assert_eq!(user.name, Some(UserName::new("TestUser").unwrap()));
...
fn test_email_lowercase_and_login_case_insensitive() {
    ...
    let new_user = NewUser::new(
        ...
        Some(UserName::new("Case").unwrap()),
        hub.id,
        UserPassword::new("pwd").unwrap(),
    );
    let user = user_repo.create_user(&new_user).unwrap();
    assert_eq!(
        user.email,
    ...
    let login = user_repo
        .login(
            &UserEmail::new("MIXED@EXAMPLE.COM").unwrap(),
            &UserPassword::new("pwd").unwrap(),
            hub.id,
        )
